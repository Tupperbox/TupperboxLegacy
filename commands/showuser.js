module.exports = {
	...
	execute: (bot, msg, args, cfg) => {
		if(!bot.recent[msg.channel.id]) bot.send(msg.channel, "No " + cfg.lang + "s have spoken in this channel since I last started up, sorry.");
			...
			bot.send(msg.channel, `Last ${cfg.lang} message sent by ${bot.recent[msg.channel.id].tulpa.name}, registered to ${user ? user.username + "#" + user.discriminator : "(unknown user " + bot.recent[msg.channel.id].user_id + ")"}`);
		}
	}
};