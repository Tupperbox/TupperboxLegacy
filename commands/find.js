...
module.exports = {
	...
	execute: async (bot, msg, args, cfg) => {
		...
		if(!tul[0])
			return bot.send(msg.channel, "Couldn't find " + article(cfg) + " " + cfg.lang + " with that name in this server.");
		if(tul.length == 1) {
			let t = tul[0];
			...
			return bot.send(msg.channel, embed);
		} else {
			let embeds = [];
			let current = { embed: {
				title: "Results",
			...
			return bot.send(msg.channel,embeds[0]);
		}
